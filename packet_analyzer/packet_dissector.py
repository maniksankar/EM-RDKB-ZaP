...
def extract_profile_type_from_autoconfig_response(filtered_packets):
    ...
    Args:
        reassembled_packets: list of reassembled packets
    ...
    if not message_presence:
        print_error("Can't detect profile : AP Autoconfiguration search message or topology query message or topology response message or bss configuration request not found in the capture file.")
    ...
    if not tlv_presence:
        print_error("Multi-AP Profile TLV not found")
...
def packet_analyzer(pcap_local_dir, ctrl_al_mac=None, extender_al_mac=None):
    ...
    for pkt in filtered_packets:
        ...
        if not message_type_name.startswith("Unknown message type"):
            ...
            print_completed_step(f"{get_message_type_name(message_type)} validation completed successfully")
